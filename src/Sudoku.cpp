...
bool Sudoku::CheckIfSafe(int row, int col, int num) {
  ...
  return (unUsedInRow(row, num) && unUsedInCol(col, num) &&
          unUsedInBox(row - row % sqrtN, col - col % sqrtN, num));
}
...
void Sudoku::mainGame(int value) {
  ...
  if (value != 0 && value != solutionGrid[pointerX][pointerY]) {
    ...
    if (incorect > 0) {
      cout << setw(4) << ""
           << printColor("CAREFULL!!! You have " + to_string(incorect) +
                             " mistakes left",
                         31);
    }
...
void Sudoku::printSudoku(int num, bool isResult) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      if (grid[i][j].getValue() == 0) {
        ...
        if (pointerX == i && pointerY == j &&
            grid[pointerX][pointerY].getValue() == 0) {
          value = "x";
        ...
        if (num != 0 && pointerX == i && pointerY == j) {
          value = to_string(num);
          isResult ? highLights[i][j] = colorTrue
                   : highLights[i][j] = colorFalse;
        }
      } else {
        if (pointerX == i && pointerY == j &&
            grid[pointerX][pointerY].getValue() != 0) {
      ...
      if (j == 0) {
        ...
        cout << printColor("|| ", colorBorder)
             << printColor(value, highLights[i][j])
             << printColor(" |", colorBorder);
        continue;
      } else if (j == 8) {
        cout << " " << printColor(value, highLights[i][j])
             << printColor(" ||", colorBorder);
      } else if (j == 2 || j == 5) {
        cout << " " << printColor(value, highLights[i][j])
             << printColor(" ||", colorBorder);
  ...
  cout << endl;
  cout << endl;
  cout << setw(2) << "" << printColor("Mistakes: ", 36) << (5 - incorect)
       << "/5" << endl;
  cout << setw(2) << "" << printColor("Progress: ", 33) << finalProgress << "%"
       << endl;
  showProgressHardMode(finalProgress);
  cout << endl << endl;
  if (incorect > 0) {
    cout << setw(2) << ""
         << printColor("Press [ESC] or [P] to pause the game", 31) << endl;
  } else if (incorect <= 0 || corect == N * N) {
    cout << setw(2) << "" << printColor("Press [ESC] or [P] to go menu", 31)
         << endl;
  }
}
...
void Sudoku::backSpaceKey() { grid[pointerX][pointerY].setvalue(0); }
